...
public class DBPediaClient extends ClientTest{
	...
	public void testSpouseNegative() {
		final Set<String> relations = Sets.newHashSet("http://dbpedia.org/ontology/occupation");
	...
	private void testMultipleRelations(boolean positive,final String statisticsFile) throws IOException {
		...
		final BufferedReader reader = new BufferedReader(new InputStreamReader(DBPediaClient.class.getResourceAsStream("all_relations.txt")));
		final Map<String,Pair<String,String>> relation2types = Maps.newHashMap();
		List<String> orderedRelations = Lists.newLinkedList();
		String line;
		...
		reader.close();
		Set<String> allRelations = Sets.newHashSet();
		AtomicInteger count = new AtomicInteger(0);
		final double startTime = System.currentTimeMillis();
		double []timeToSubtract = new double[] {0};
		orderedRelations.forEach( relation -> {
			double curStartTime = System.currentTimeMillis();
			count.incrementAndGet();
			LOGGER.info("Starting computation for relation '{}', {} out of {}.",relation,count.get(),orderedRelations.size());
			final String id = relation;
			StatisticsContainer.initialiseContainer(id);
			allRelations.clear();
			allRelations.add(relation);
			Pair<String,String> subjectObject = relation2types.get(relation);
			try {
				if(positive) {
					Assert.assertNotNull(super.executeRudikPositiveRules(allRelations, subjectObject.getLeft(), subjectObject.getRight(),null,null)); 
				} else {
					Assert.assertNotNull(super.executeRudikNegativeRules(allRelations, subjectObject.getLeft(), subjectObject.getRight(),null,null)); 
				}
				try {
			...
			LOGGER.info("Current running time: '{}' seconds.",(System.currentTimeMillis()-startTime-timeToSubtract[0])/1000.);
		});
	}
