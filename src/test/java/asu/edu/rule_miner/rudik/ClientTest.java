...
public class ClientTest 
{
	...
	protected List<HornRule> executeRudikNegativeRules(final Set<String> relationNames, final String subjectType, final String objectType, Set<Pair<String,String>> posExamples, Set<Pair<String,String>> negExamples){
		...
		final List<HornRule> outputRules = rudik.
				discoverNegativeHornRules(negExamples, posExamples, relationNames, subjectType, objectType);
	...
	protected List<HornRule> executeRudikPositiveRules(final Set<String> relationNames, final String subjectType, final String objectType, Set<Pair<String,String>> positiveExamples, Set<Pair<String,String>> negativeExamples){
		final Instant startTime = Instant.now();

		//get positive and negative examples
		if(negativeExamples == null) {
			negativeExamples = rudik.
					generateNegativeExamples(relationNames, subjectType, objectType, false, false);
		}
		if(positiveExamples == null) {
			positiveExamples = rudik.
					generatePositiveExamples(relationNames, subjectType, objectType);
		}
		StatisticsContainer.setGenerationSample(positiveExamples);

		//compute outputs
		final List<HornRule> outputRules = rudik.discoverPositiveHornRules
				(negativeExamples, positiveExamples, relationNames, subjectType, objectType);
